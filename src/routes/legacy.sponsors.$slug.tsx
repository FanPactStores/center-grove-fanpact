...
export const Route = createFileRoute("/legacy/sponsors/$slug")({
  loader: ({ params }) => {
...
function NotFoundView() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-24 text-center lg:px-8">
      ...
      <Link to="/legacy/sponsors" className="mt-6 inline-block text-sm underline">
        All sponsors
      </Link>
    </main>
  );
...
function SponsorRouter() {
  ...
  if (enterpriseSlug === "edward-jones") {
    return <EdwardJonesPage store={STORES["legacy"]} variant="youth" />;
  }
  const enterprise = enterpriseSlug ? getEnterprisePartner(enterpriseSlug) : null;
  if (enterprise) {
    return <EnterpriseSponsorPage store={STORES["legacy"]} partner={enterprise} />;
  }
