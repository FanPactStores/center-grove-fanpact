        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_1fr]">
            ...
            <div className="mt-6 space-y-3">
              {sponsor.actions.map((a: typeof sponsor.actions[number], i: number) => (
                <div key={a.title} className="flex items-center justify-between rounded-xl border border-border bg-card p-5">
                  ...
                  <div className="font-display text-2xl tracking-tight" style={{ color: "var(--community)" }}>+{usd(a.reward)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <aside className="rounded-2xl border border-border bg-card p-6">
            ...
            <Button asChild size="lg" className="mt-6 w-full" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }}>
              <Link to="/assa/sponsors/$slug/claim" params={{ slug: sponsor.slug }}>
                Claim Your Credit
              </Link>
            </Button>
