...
export const Route = createFileRoute("/missouri/shop/$category")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.category.name ?? "Category"} — Missouri × FanPact` },
      { name: "description", content: loaderData?.category.blurb ?? "" },
    ],
  }),
  errorComponent: () => <ErrorView />,
  notFoundComponent: () => <ErrorView />,
  component: MissouriCategory,
});
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-24 text-center lg:px-8">
      ...
      <p className="mt-3 text-muted-foreground">
        That category doesn't exist in the Missouri store.
      </p>
      <Link to="/missouri/shop" className="mt-6 inline-block text-sm underline">
    ...
    <main className="bg-background">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-xs text-muted-foreground">
          <Link to="/missouri" className="hover:text-foreground">
            Store
          </Link>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[240px_1fr]">
          {/* FILTERS SIDEBAR */}
          <aside
            className={`${filtersOpen ? "block" : "hidden"} lg:block lg:sticky lg:top-20 lg:self-start`}
          >
          ...
          <div>
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="text-sm">
                Showing <span className="font-bold">{visible.length}</span> products
              </div>
              <div className="ml-auto flex items-center gap-2">
                <div className="hidden items-center overflow-hidden rounded-md border border-border md:flex">
                  <button
                    ...
                    className="p-2"
                    style={{
                      background: view === "grid" ? "var(--brand-accent)" : "transparent",
                  ...
                  <button
                    ...
                    className="p-2"
                    style={{
                      background: view === "list" ? "var(--brand-accent)" : "transparent",
            ...
            {visible.length === 0 ? (
              ...
              <div className="divide-y divide-border rounded-xl border border-border bg-card">
                {visible.map((p) => (
                  <Link
                    key={p.id}
                    to="/missouri/product/$slug"
                    params={{ slug: p.slug }}
                    className="flex items-center gap-4 p-4 transition-colors hover:bg-muted/50"
                  >
                    <img
                      src={productImage(p, 240)}
                      alt={p.name}
                      width={96}
                      height={96}
                      loading="lazy"
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">
                        +{usd(p.contribution)} community contribution
                      </div>
                    </div>
