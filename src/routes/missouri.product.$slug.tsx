...
export const Route = createFileRoute("/missouri/product/$slug")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.product.name ?? "Product"} — Missouri × FanPact` },
            ...
            {tab === "description" && (
              <div className="space-y-4">
                ...
                <p className="text-muted-foreground">
                  Every purchase of {product.brand} {product.name} through the Missouri
                  FanPact storefront contributes {usd(product.contribution)} to {store.fundName},
                  helping fund NIL opportunities for Missouri student-athletes.
                </p>
              </div>
            )}
            {tab === "specifications" && (
