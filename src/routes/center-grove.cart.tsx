...
export const Route = createFileRoute("/center-grove/cart")({
  head: () => ({ meta: [{ title: "Cart — Center Grove × FanPact" }] }),
  component: ButlerCart,
});
...
function ButlerCart() {
  ...
  const { designation, set: setDesignation } = useDesignation("center-grove");
    ...
    <main className="mx-auto max-w-7xl px-4 py-12 lg:px-8">
      ...
      {items.length === 0 ? (
        <div className="mt-12 rounded-xl border border-dashed border-border p-12 text-center">
          ...
          <Button asChild className="mt-4"><Link to="/center-grove/shop">Browse the store</Link></Button>
        </div>
      ) : (
            ...
            {items.map(({ product, qty }) => (
              <div key={product.slug} className="flex gap-4 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <Link to="/center-grove/product/$slug" params={{ slug: product.slug }} className="line-clamp-1 text-sm font-medium hover:underline">
                    {product.name}
                  </Link>
                  <div className="mt-1 text-xs font-medium" style={{ color: "var(--community)" }}>
                    +{usd(product.contribution * qty)} community contribution
                  </div>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <Button
                ...
                onClick={() =>
                  navigate({
                    to: "/center-grove/checkout-confirmation",
              ...
              <p className="mt-3 text-center text-[11px] text-muted-foreground">
                Demo checkout — no payment will be captured.
              </p>
            </div>
          </aside>
        </div>
...
function Row({ label, value, large, accent }: { label: string; value: string; large?: boolean; accent?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span
        className={large ? "font-display text-2xl tracking-tight" : "tabular-nums"}
