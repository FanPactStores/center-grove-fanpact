...
export const Route = createFileRoute("/missouri/sponsors/$slug")({
  loader: ({ params }) => {
...
function NotFoundView() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-24 text-center lg:px-8">
      ...
      <Link to="/missouri/sponsors" className="mt-6 inline-block text-sm underline">
        All sponsors
      </Link>
    </main>
  );
...
function SponsorRouter() {
  ...
  if (enterpriseSlug === "edward-jones") {
    return <EdwardJonesPage store={STORES["missouri"]} variant="collegiate" />;
  }
  const enterprise = enterpriseSlug ? getEnterprisePartner(enterpriseSlug) : null;
  if (enterprise) {
    return <EnterpriseSponsorPage store={STORES["missouri"]} partner={enterprise} />;
  }
    ...
    <main>
      <section className="border-b border-border" style={{ background: sponsor.color, color: "white" }}>
        <div className="mx-auto max-w-7xl px-4 py-20 lg:px-8">
          <Link to="/missouri/sponsors" className="text-xs uppercase tracking-[0.2em] opacity-80 hover:opacity-100">← All sponsors</Link>
        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_1fr]">
          ...
          <aside className="rounded-2xl border border-border bg-card p-6">
            ...
            <Button asChild size="lg" className="mt-6 w-full" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }}>
              <Link to="/missouri/sponsors/$slug/claim" params={{ slug: sponsor.slug }}>
                Claim Your Credit
              </Link>
            </Button>
