...
export const Route = createFileRoute("/cmn/st-johns/shop/")({
  validateSearch: (search: Record<string, unknown>): ShopSearch => ({
    search: typeof search.search === "string" && search.search ? search.search : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Shop — HSHS St. John's Children's Hospital × FanPact" },
      {
        name: "description",
        content:
          "Browse every product. 60% of net earnings flows to HSHS St. John's Children's Hospital.",
      },
      { property: "og:title", content: "Shop — HSHS St. John's Children's Hospital × FanPact" },
      {
...
function StJohnsShop() {
  ...
  const setQuery = (v: string) => {
    navigate({ to: "/cmn/st-johns/shop", search: v ? { search: v } : {}, replace: true });
  };
