...
export const Route = createFileRoute("/missouri/sponsors/$slug/claim")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `Credit claimed — ${loaderData?.sponsor.name ?? "Sponsor"} × Missouri` },
...
function NotFoundView() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-24 text-center lg:px-8">
      ...
      <Link to="/missouri/sponsors" className="mt-6 inline-block text-sm underline">All sponsors</Link>
    </main>
  );
}
    ...
    <main className="mx-auto max-w-3xl px-4 py-20 lg:px-8">
      <div className="rounded-3xl border border-border bg-card p-10 text-center shadow-sm">
        ...
        <p className="mt-4 text-muted-foreground">
          ...
          will land in your Missouri community account automatically — no purchase, no commitment.
        </p>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <Button asChild size="lg" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }}>
            <Link to="/missouri/teams">Designate a Tiger</Link>
          </Button>
          <Button asChild variant="outline" size="lg">
            <Link to="/missouri/sponsors/$slug" params={{ slug: sponsor.slug }}>← Back to campaign</Link>
          </Button>
        </div>
