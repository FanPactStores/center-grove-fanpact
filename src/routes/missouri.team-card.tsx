...
export const Route = createFileRoute("/missouri/team-card")({
  head: () => ({
    meta: [
      { title: "Team Card — Missouri × FanPact" },
...
function Cardholder({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  return (
    ...
    >
      ...
      <Field label="Full legal name" placeholder="Jane Tiger" />
...
function Done({ store, onReset }: { store: typeof STORES["missouri"]; onReset: () => void }) {
  return (
