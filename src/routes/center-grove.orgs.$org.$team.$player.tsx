...
import { getCGOrg, getCGTeam, getCGPlayer } from "@/data/center-grove-orgs";
import { Button } from "@/components/ui/button";
import { usd } from "@/lib/format";

export const Route = createFileRoute("/center-grove/orgs/$org/$team/$player")({
  loader: ({ params }) => {
    const org = getCGOrg(params.org);
    const team = getCGTeam(params.org, params.team);
    const player = getCGPlayer(params.org, params.team, params.player);
    if (!org || !team || !player) throw notFound();
    return { org, team, player };
  },
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.player.name ?? "Player"} — Center Grove × FanPact` },
      { name: "description", content: `${loaderData?.player.name}, #${loaderData?.player.number}, ${loaderData?.player.position}. Designate this athlete and your community contributions flow directly to them.` },
    ],
  }),
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 lg:px-8">
      <nav className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
        <Link to="/center-grove/orgs" className="hover:text-foreground">Organizations</Link>
        ...
        <div>
          ...
          >
            ...
            <p className="mt-2 text-sm text-foreground/80">
              Designate {firstName} as your beneficiary and 60% of net earnings on every purchase
              you make at the Center Grove store flows directly to their community account.
            </p>
          ...
          <div className="mt-8 border-t border-border pt-6 text-sm text-muted-foreground">
            <p>
              Bio coming soon. {player.name} plays for {team.name} ({team.ageGroup}) in the {org.name} program.
              Coach: {team.coach}.
            </p>
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button asChild variant="ghost">
              <Link to="/center-grove/shop">Shop the store →</Link>
            </Button>
          </div>
        </div>
      </div>
