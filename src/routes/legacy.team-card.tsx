...
export const Route = createFileRoute("/legacy/team-card")({
  head: () => ({
    meta: [
      ...
      <div className="grid gap-12 lg:grid-cols-[1.1fr_1fr]">
        <div>
          ...
          <p className="mt-5 max-w-md text-muted-foreground">
            A Stripe-issued Visa debit in Apple Pay and Google Pay. Spend you'd already make earns contributions for {store.fundDisplay}.
          </p>
        ...
        <div className="rounded-2xl border border-border bg-card p-8">
          <div className="mb-6 flex items-center gap-2 text-xs uppercase tracking-[0.18em] text-muted-foreground">
            {steps.map((s, i) => (
              <span key={s} className="flex items-center gap-2">
                <span className={`flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-semibold ${i === step ? "text-white" : i < step ? "text-[var(--community)]" : "text-muted-foreground"}`} style={{ background: i === step ? "var(--brand)" : i < step ? "var(--community-soft)" : "var(--muted)" }}>
          ...
          {step === 1 && <Cardholder onNext={() => setStep(2)} onBack={() => setStep(0)} />}
          {step === 2 && <Wallet onNext={() => setStep(3)} onBack={() => setStep(1)} />}
          {step === 3 && <Done store={store} onReset={() => setStep(0)} />}
        </div>
      </div>
    </main>
  );
...
function Intro({ onNext }: { onNext: () => void }) {
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-muted-foreground">Takes about 90 seconds. No credit pull. No card mailed.</p>
      <div className="mt-6 grid gap-3">
        {[
          { icon: Sparkles, t: "Step 1 — Tell us about you", d: "Name, date of birth, last 4 of SSN for ID verification." },
          { icon: Smartphone, t: "Step 2 — Add to Apple or Google Pay", d: "One tap. Card provisioned to your wallet instantly." },
      ...
      <Button size="lg" className="mt-6 w-full" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }} onClick={onNext}>Start enrollment</Button>
    </div>
  );
}
...
function Cardholder({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  return (
    <form onSubmit={(e) => { e.preventDefault(); onNext(); }} className="space-y-4">
      ...
      <Field label="Full legal name" placeholder="Jane Smith" />
      ...
      <Field label="Mobile" placeholder="(314) 555-0102" />
      <div className="flex gap-2 pt-2">
        <Button type="button" variant="outline" onClick={onBack}>Back</Button>
        <Button type="submit" className="flex-1" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }}>Continue</Button>
      </div>
    </form>
  );
}
...
function Wallet({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  return (
    <div>
      ...
      <div className="mt-6 grid gap-3">
        <button onClick={onNext} className="flex items-center justify-between rounded-lg border border-border bg-foreground p-4 text-background transition-all hover:scale-[1.01]">
          <span className="text-sm font-semibold"> Add to Apple Wallet</span>
          <Smartphone className="h-5 w-5" />
        </button>
        <button onClick={onNext} className="flex items-center justify-between rounded-lg border border-border p-4 transition-all hover:bg-muted">
...
function Done({ store, onReset }: { store: typeof STORES["legacy"]; onReset: () => void }) {
  return (
    <div className="text-center">
      ...
      <p className="mt-2 text-sm text-muted-foreground">Your Team Card is in your wallet. Every tap from now on contributes to {store.fundName}.</p>
...
function Field({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <label className="block">
      ...
      <input {...props} className="mt-1.5 block w-full rounded-md border border-border bg-background px-3 py-2 text-sm shadow-sm outline-none focus:ring-2 focus:ring-[var(--brand)]" />
    </label>
  );
}
