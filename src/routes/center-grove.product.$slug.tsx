...
export const Route = createFileRoute("/center-grove/product/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.product.name ?? "Product"} — Center Grove × FanPact` },
            ...
            {tab === "description" && (
              <div className="space-y-4">
                ...
                <p className="text-muted-foreground">
                  Every purchase of {product.brand} {product.name} through the Center Grove
                  FanPact storefront contributes {usd(product.contribution)} to {store.fundName},
                  helping fund NIL opportunities for Center Grove student-athletes.
                </p>
              </div>
            )}
            {tab === "specifications" && (
