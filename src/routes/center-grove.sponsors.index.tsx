...
export const Route = createFileRoute("/center-grove/sponsors/")({
  head: () => ({
    meta: [
      { title: "Enterprise sponsors — Center Grove × FanPact" },
      { name: "description", content: "Brands prepay community accounts. Families unlock credits by completing qualifying actions." },
    ],
  }),
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-12 lg:px-8">
      <div className="max-w-2xl">
        ...
        <h1 className="mt-2 font-display text-5xl tracking-tight">Brands fund Center Grove families.</h1>
        <p className="mt-4 text-muted-foreground">
          Sponsors prepay community accounts. Families unlock credits when they complete qualifying
          actions — no purchase required. Every campaign here is live.
        </p>
      </div>

      <div className="mt-12 grid gap-6 lg:grid-cols-3">
        {sponsorsForStore("center-grove").map((s) => {
          const pct = Math.round((s.fundReleased / s.fundTotal) * 100);
          return (
            <Link
              key={s.slug}
              to="/center-grove/sponsors/$slug"
              params={{ slug: s.slug }}
              className="group rounded-2xl border border-border bg-card p-8 transition-all hover:-translate-y-1 hover:shadow-xl"
            >
              <div
                className="mb-6 inline-flex h-10 items-center justify-center rounded-md px-3 font-display text-sm tracking-wider text-white"
