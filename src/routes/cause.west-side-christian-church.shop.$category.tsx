...
export const Route = createFileRoute("/cause/west-side-christian-church/shop/$category")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      {
        title: `${loaderData?.category.name ?? "Category"} — West Side Christian Church × FanPact`,
      },
      { name: "description", content: loaderData?.category.blurb ?? "" },
      { property: "og:title", content: `${loaderData?.category.name ?? "Shop"} — West Side Christian Church` },
      { property: "og:description", content: loaderData?.category.blurb ?? "" },
    ],
  }),
  errorComponent: () => <ErrorView />,
  notFoundComponent: () => <ErrorView />,
  component: WestSideCategory,
});
...
function ErrorView() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-24 text-center lg:px-8">
      ...
      <Link to="/cause/west-side-christian-church/shop" className="mt-6 inline-block text-sm underline">
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-12 lg:px-8">
      <nav className="flex items-center gap-1 text-xs text-muted-foreground">
        <Link to="/cause/west-side-christian-church/shop" className="hover:underline">
          Shop
        </Link>
