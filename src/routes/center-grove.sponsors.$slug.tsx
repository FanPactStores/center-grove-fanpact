...
export const Route = createFileRoute("/center-grove/sponsors/$slug")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.sponsor.name ?? "Sponsor"} — Center Grove × FanPact` },
    ...
    <main>
      <section className="border-b border-border" style={{ background: sponsor.color, color: "white" }}>
        <div className="mx-auto max-w-7xl px-4 py-20 lg:px-8">
          <Link to="/center-grove/sponsors" className="text-xs uppercase tracking-[0.2em] opacity-80 hover:opacity-100">← All sponsors</Link>
        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_1fr]">
          ...
          <aside className="rounded-2xl border border-border bg-card p-6">
            ...
            <Button asChild size="lg" className="mt-6 w-full" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }}>
              <Link to="/center-grove/sponsors/$slug/claim" params={{ slug: sponsor.slug }}>
                Claim Your Credit
              </Link>
            </Button>
