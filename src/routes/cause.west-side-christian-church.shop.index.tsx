...
export const Route = createFileRoute("/cause/west-side-christian-church/shop/")({
  validateSearch: (search: Record<string, unknown>): ShopSearch => ({
    search: typeof search.search === "string" && search.search ? search.search : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Shop — West Side Christian Church × FanPact" },
      {
        name: "description",
        content:
          "Browse every product. 60% of net earnings flows to West Side Christian Church.",
      },
  ...
  component: WestSideShop,
});

function WestSideShop() {
  ...
  const setQuery = (v: string) => {
    navigate({ to: "/cause/west-side-christian-church/shop", search: v ? { search: v } : {}, replace: true });
  };
