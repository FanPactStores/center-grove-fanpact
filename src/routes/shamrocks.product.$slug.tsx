...
export const Route = createFileRoute("/shamrocks/product/$slug")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.product.name ?? "Product"} — Springfield Shamrocks × FanPact` },
...
function ButlerProduct() {
  ...
  const { isOnList, addProduct, removeProduct } = useMyList("shamrocks");
            ...
            {tab === "description" && (
              <div className="space-y-4">
                ...
                <p className="text-muted-foreground">
                  Every purchase of {product.brand} {product.name} through the Springfield Shamrocks
                  FanPact storefront contributes {usd(product.contribution)} to {store.fundName},
                  helping fund NIL opportunities for Springfield Shamrocks student-athletes.
                </p>
              </div>
            )}
            {tab === "specifications" && (
