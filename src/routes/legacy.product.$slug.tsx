...
import { ChevronRight, Check, Minus, Plus, ShoppingCart, Truck, ShieldCheck, RotateCcw } from "lucide-react";
...
export const Route = createFileRoute("/legacy/product/$slug")({
  loader: ({ params }) => {
...
function LegacyProduct() {
  ...
  const { isOnList, addProduct, removeProduct } = useMyList("legacy");
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        ...
        <div className="mt-8 grid gap-12 lg:grid-cols-[1.1fr_1fr]">
          <div>
            <div className="overflow-hidden rounded-2xl border border-border bg-white">
              <img src={productImage(product, 1200, activeImg)} alt={product.name} width={1200} height={1200} className="aspect-square w-full object-contain p-8" />
            </div>
            <div className="mt-4 grid grid-cols-5 gap-3">
              {[0, 1, 2, 3, 4].map((v) => (
                <button
                  ...
                  style={{ borderColor: activeImg === v ? "var(--brand-accent)" : "var(--border)" }}
                  aria-label={`View image ${v + 1}`}
                >
                  <img src={productImage(product, 240, v)} alt={`${product.name} thumbnail ${v + 1}`} width={240} height={240} loading="lazy" className="aspect-square w-full object-contain" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground">SKU · {product.sku}</div>
            <div className="mt-2 text-xs uppercase tracking-[0.22em] text-muted-foreground">{product.brand}</div>
            <h1 className="mt-2 font-display text-4xl leading-tight tracking-tight md:text-5xl">{product.name}</h1>

            <div className="mt-6 flex items-baseline gap-4">
              <span className="font-display text-5xl tracking-tight" style={{ color: "var(--brand-accent)" }}>{usd(product.price)}</span>
            </div>

            <div className="mt-3 text-sm font-semibold" style={{ color: "var(--brand-accent)" }}>
              FanPact Community Contribution: {usd(product.contribution * qty)}
            </div>

            <div className="mt-6 inline-flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm" style={{ borderColor: product.inStock ? "var(--community-soft)" : "var(--border)", background: product.inStock ? "color-mix(in oklab, var(--community) 8%, transparent)" : "var(--muted)" }}>
              <span className="h-2 w-2 rounded-full" style={{ background: product.inStock ? "var(--community)" : "oklch(0.6 0.18 27)" }} />
              <span className="font-semibold">{product.inStock ? "In stock — ships fast" : "Out of stock"}</span>
            </div>
            ...
            <div className="mt-8 flex items-stretch gap-3">
              <div className="flex items-center rounded-md border border-border">
                <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="p-3 hover:bg-muted" aria-label="Decrease" disabled={!product.inStock}>
                  <Minus className="h-4 w-4" />
                </button>
                <span className="w-12 text-center text-sm font-semibold tabular-nums">{qty}</span>
                <button onClick={() => setQty((q) => q + 1)} className="p-3 hover:bg-muted" aria-label="Increase" disabled={!product.inStock}>
              ...
              <button
                ...
                style={{ background: product.inStock ? "var(--brand-accent)" : "var(--muted)", color: product.inStock ? "white" : "var(--muted-foreground)" }}
              >
                {!product.inStock ? "Out of Stock" : added ? (<><Check className="h-5 w-5" /> Added to Cart</>) : (<><ShoppingCart className="h-5 w-5" /> Add to Cart — {usd(product.price * qty)}</>)}
              </button>
            </div>
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 border-t border-border pt-6">
              {[
                ...
                <div key={b.t} className="text-center">
                  <b.icon className="mx-auto h-5 w-5" style={{ color: "var(--brand-accent)" }} />
        ...
        <section className="mt-16">
          <div className="flex gap-6 border-b border-border">
            {([["description", "Description"], ["specifications", "Specifications"], ["shipping", "Shipping"]] as [TabKey, string][]).map(([k, label]) => (
              <button key={k} onClick={() => setTab(k)} className="relative -mb-px py-3 text-sm font-semibold transition-colors" style={{ color: tab === k ? "var(--foreground)" : "var(--muted-foreground)", borderBottom: tab === k ? `2px solid var(--brand-accent)` : "2px solid transparent" }}>
                {label}
              </button>
            ))}
          </div>
              ...
              <table className="w-full overflow-hidden rounded-lg border border-border text-sm">
                <tbody className="divide-y divide-border">
                  {product.specs.map((s: { label: string; value: string }) => (
                    <tr key={s.label}>
                      <td className="w-1/3 bg-muted/50 px-4 py-3 font-semibold">{s.label}</td>
                      <td className="px-4 py-3 text-foreground/80">{s.value}</td>
                    </tr>
                  ))}
                  <tr><td className="w-1/3 bg-muted/50 px-4 py-3 font-semibold">Brand</td><td className="px-4 py-3 text-foreground/80">{product.brand}</td></tr>
                  <tr><td className="w-1/3 bg-muted/50 px-4 py-3 font-semibold">SKU</td><td className="px-4 py-3 font-mono text-xs text-foreground/80">{product.sku}</td></tr>
                </tbody>
              </table>
            )}
            {tab === "shipping" && (
              <div className="space-y-3">
                <p><strong>Free standard shipping</strong> on orders over $35. Orders ship within 1–2 business days.</p>
                <p>Expedited and 2-day shipping options are available at checkout.</p>
