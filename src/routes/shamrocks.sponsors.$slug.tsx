...
export const Route = createFileRoute("/shamrocks/sponsors/$slug")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: `${name} — Springfield Shamrocks × FanPact` },
...
function NotFoundView() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-24 text-center lg:px-8">
      ...
      <Link to="/shamrocks/sponsors" className="mt-6 inline-block text-sm underline">
        All sponsors
      </Link>
    </main>
  );
...
function SponsorRouter() {
  ...
  if (enterpriseSlug === "edward-jones") {
    return <EdwardJonesPage store={STORES["shamrocks"]} variant="youth" />;
  }
  const enterprise = enterpriseSlug ? getEnterprisePartner(enterpriseSlug) : null;
  if (enterprise) {
    return <EnterpriseSponsorPage store={STORES["shamrocks"]} partner={enterprise} />;
  }
    ...
    <main>
      <section className="border-b border-border" style={{ background: sponsor.color, color: "white" }}>
        <div className="mx-auto max-w-7xl px-4 py-20 lg:px-8">
          <Link to="/shamrocks/sponsors" className="text-xs uppercase tracking-[0.2em] opacity-80 hover:opacity-100">← All sponsors</Link>
        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_1fr]">
          ...
          <aside className="rounded-2xl border border-border bg-card p-6">
            ...
            <Button asChild size="lg" className="mt-6 w-full" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }}>
              <Link to="/shamrocks/sponsors/$slug/claim" params={{ slug: sponsor.slug }}>
                Claim Your Credit
              </Link>
            </Button>
