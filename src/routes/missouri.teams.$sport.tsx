...
import { getMissouriTeam } from "@/data/missouri-teams";

export const Route = createFileRoute("/missouri/teams/$sport")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.team.name ?? "Team"} roster — Missouri × FanPact` },
    ...
    <main>
      <section className="border-b border-border" style={{ background: "var(--brand)", color: "var(--brand-foreground)" }}>
        <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
          <Link to="/missouri/teams" className="text-xs uppercase tracking-[0.2em] opacity-80 hover:opacity-100">← All teams</Link>
