...
export const Route = createFileRoute("/legacy/orgs/$org/$team")({
  loader: ({ params }) => {
          ...
          <div className="mt-4 flex flex-wrap items-end justify-between gap-6">
            ...
            <div className="flex flex-col items-end gap-3">
              <div className="text-right">
                <div className="font-display text-6xl tracking-tight" style={{ color: "var(--brand-accent)" }}>
                  {team.players.length}
                </div>
                <div className="text-xs uppercase tracking-widest opacity-70">Players</div>
              </div>
              <Button size="lg" style={{ background: "var(--brand-accent)", color: "white" }}>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {team.players.map((p: LegacyPlayer) => (
            ...
            >
              ...
              <div className="p-3">
                <div className="font-display text-base leading-tight group-hover:underline">{p.name}</div>
                <div className="mt-1 text-[11px] text-muted-foreground">{p.grade} · {p.hometown}</div>
              </div>
            </Link>
          ))}
        </div>
