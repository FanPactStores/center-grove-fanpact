...
export const Route = createFileRoute("/missouri/cart")({
  head: () => ({ meta: [{ title: "Cart — Missouri × FanPact" }] }),
  component: MissouriCart,
});

function MissouriCart() {
  ...
  const { designation, set: setDesignation } = useDesignation("missouri");
    ...
    <main className="mx-auto max-w-7xl px-4 py-12 lg:px-8">
      ...
      {items.length === 0 ? (
        <div className="mt-12 rounded-xl border border-dashed border-border p-12 text-center">
          ...
          <Button asChild className="mt-4"><Link to="/missouri/shop">Browse the store</Link></Button>
        </div>
      ) : (
            ...
            {items.map(({ product, qty }) => (
              <div key={product.slug} className="flex gap-4 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <Link to="/missouri/product/$slug" params={{ slug: product.slug }} className="line-clamp-1 text-sm font-medium hover:underline">
                    {product.name}
                  </Link>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <Button
                ...
                onClick={() =>
                  navigate({
                    to: "/missouri/checkout-confirmation",
